# 狩猟免許（網猟／わな猟／第一種銃猟／第二種銃猟）学習サイト

静的HTML/CSS/JSのみで作られた、ビルド不要の学習サイトです。
`index.html` をダブルクリックするだけで動作します（サーバー不要）。
知識試験・模擬試験は「免許種別」を網猟／わな猟／第一種銃猟／第二種銃猟の4種類から選択でき、
猟具の出題内容が選んだ免許に応じて切り替わります。
知識試験は分野に加えて「論点」でも絞り込めるほか、論点別の正答率を記録して弱点を可視化します。
...
```js
{
  ...
  category: "law",          // "law"(法令) / "gear"(猟具) / "knowledge"(鳥獣) / "management"(保護管理)
  license: ["net", "trap", "gun1", "gun2"],  // この問題を出題する免許。1つ以上
  point: "(8)捕獲規制区域等",  // 例題集「知識試験例題の出題ポイント一覧」の分類
  question: "問題文をここに書く。",
  choices: ["選択肢1", "選択肢2", "選択肢3"],  // 必ず3択
  answer: 0,                 // 正解のインデックス（0始まり。上の例なら「選択肢1」が正解）
  explanation: "正解の根拠となる解説文。復習画面・模擬試験の結果画面に表示される。",
  source: "法第11条第1項",   // 根拠条文・出典
  verified: "2026-08-12",    // 最終確認日。法改正時に古い問題を洗い出すために使う
},
```

- `category` は本試験の出題4分野 "law"(法令) / "gear"(猟具) / "knowledge"(鳥獣) /
  "management"(保護管理) の4種類です。
- **免許ごとの出し分けは `category` ではなく `license` が担います。**
  値は "net"(網猟) / "trap"(わな猟) / "gun1"(第一種銃猟) / "gun2"(第二種銃猟)。
  法令・鳥獣・保護管理の多くは4免許共通なので4つすべてを、猟具の問題は該当する免許だけを
  指定してください。銃器の取扱いのように第一種・第二種で共通の内容は両方を指定します。
- 出題は `QUESTIONS.filter(q => q.license.includes(選択免許) && q.category === 分野)` で絞り込みます。
- 模擬試験モードは `common.js` の `MOCK_DISTRIBUTION`（法令13・猟具6・鳥獣9・保護管理2＝計30問）に
  従って抽出します。**免許ごとに、4分野それぞれの問題数がこの数を下回らないようにしてください。**
